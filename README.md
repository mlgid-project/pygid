...
x, y, img = analysis.plot_img_raw(
                                 ...
                                 clims=(0.1, 100),  # tuple specifying color limits (vmin, vmax) for the image.
...
```python
analysis.det2q_gid(frame_num=[0, 1, 2],# frame number or list of numbers of loaded raw images to convert. If None, will convert all loaded images 
...
q, intensity = analysis.radial_profile(plot_result = True, return_result = True)
chi, intensity = analysis.azim_profile(plot_result = True, return_result = True)
